              ...
              (tx, results) => {
                ...
                if(results.rowsAffected>0){
                  Alert.alert( 'Success', 'User updated successfully',
                    [
                      {text: 'Ok', onPress: () => that.props.navigation.navigate('Home1')},
                    ],
                    { cancelable: false }
                  );
                }else{
                  alert('เกิดข้อผิดพลาดกรุณาแก้ไขข้อมูลใหม่');
                }
              }
            );
          });
        }else{
          alert('กรุณากรอกUserID');
        }
      }else{
        alert('กรุณากรอกขนาดห้อง');
      }
    }else{
      alert('กรุณรากรอกวันและเวลา');
    }
  };
      ...
      <View style={{ backgroundColor: 'white', flex: 1 }}>
        <ScrollView keyboardShouldPersistTaps="handled">
          <KeyboardAvoidingView
            ...
            <Mytextinput
              placeholder="กรอกUserIDที่จะแก้ไขข้อมูล"
              style={{ padding:10 }}
              onChangeText={input_user_id => this.setState({ input_user_id })}
            />
            <Mybutton
              title="ค้นหาข้อมูลUserID"
            ...
            />
            <Mytextinput
              placeholder="กรอกขนาดห้อง(L,M,S)"
            ...
            />
            <Mybutton
              title="ยืนยันการแก้ไขข้อมูลการจองห้อง"
